...
If we want to create a group with an element $x$, we get the group $\langle x \rangle
 = \{e, x, x^{-1}, x^2, x^{-2}, x^3, x^{-3}, .....,\}$. The elements keep appearing for the group axioms, multiplication is closed under addition so $x \in G \implies x \circ x \in G$, and similarly $x^3,.. x^n$ is in $G$. Also the inverses get included. Such groups are known as cyclic groups. Relations, generators and presentations will be discussed later properly.
