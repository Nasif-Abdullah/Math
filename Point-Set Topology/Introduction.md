...
If a set $$U$$ is not finite, here, $$Finiteness(U) = T$$. Now we have-
 ...
 - Similarly, $$(\forall i \in I) \ \ \phi(U_i)$$. If $$(\exists i) \ \ $$ such that $$U_i = \emptyset$$, then $$\bigcap{i=1}^n U_i = \emptyset \implies \phi(\bigcap_{i=1}^n U_i)$$. Otherwise, $$(\forall i) \ \ |X \setminus U_i| < \infty$$. $$X \setminus \left(\bigcap_{i=1}^n U_i\right) = \bigcup_{i=1}^n (X \setminus U_i)$$ (De Morgan). Since a finite union of finite sets is finite, $$\phi(\bigcap_{i=1}^n U_i)$$ holds. $$\square$$
